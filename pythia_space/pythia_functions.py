...
def get_objective_func(params, metric):
    ...
    elif metric == 'entropy' or metric == 'mod-log-likelihood':
        m = get_scipy_hist_metric(blocks,object_contents,object_errors,tune_contents,tune_errors, metric)
    elif metric == 'ks_2samp' or metric == 'wasserstein':
        m = get_scipy_cts_metric(blocks,object_contents,object_errors,tune_contents,tune_errors, metric)
...
def Get_Ratio_Error_Numbers(num,denom):
    ...
    if num == 0 and denom != 0:
        error = value * math.sqrt(1./denom)
    else:
        error = value * math.sqrt(1./num + 1./denom)
    return error
...
def get_scipy_hist_metric(blocks,object_contents,object_errors,tune_contents,tune_errors, metric):
    powerDivergence = ['pearson', 'log-likelihood', 'freeman-tukey', 'mod-log-likelihood', 'neyman', 'cressie-read']
    doubleSign = ['log-likelihood', 'freeman-tukey', 'mod-log-likelihood', 'neyman', 'cressie-read']
    if metric in powerDivergence:
        metricFnc = getattr(scipy.stats, 'power_divergence')
    else:
        metricFnc = getattr(scipy.stats, metric)
...
#                               quotechar=" ",lineterminator="\n",quoting=csv.QUOTE_ALL)
    ...
    for ihist in range(len(lists[0])):
        ...
        for i in range(len(indicesToRemove)):
            del true[indicesToRemove[i] - i]
            del fit[indicesToRemove[i] - i]
        m_val = metricFnc(fit, true)
        if metric in powerDivergence:
            m_val = m_val[0]
        if metric in doubleSign:
            m_val = abs(m_val)
        m_list.append(m_val)
...
#     ofile_writer.writerow(m_list)
#     ofile.close()
    
    return result

def get_scipy_cts_metric(blocks,object_contents,object_errors,tune_contents,tune_errors, metric):
    metricFnc = getattr(scipy.stats, metric)
    ...
    for ihist in range(len(lists[0])):
        ...
        for ibin in range(len(lists[0][ihist])):
            ...
            if np.isnan(true_value):
                true_value = 0
            fit += int(fit_value*samples) * [ibin]
            true += int(true_value*samples) * [ibin]
        m_val = metricFnc(fit, true)
        if metric == 'ks_2samp':
            m_val = m_val[0]
        m_list.append(m_val)
...
def get_chi2(blocks,object_contents,object_errors,tune_contents,tune_errors):
